...
def get_open_incidents() -> list[dict]:
    ...
    cur.execute("SELECT inc_id, title, severity, ai_summary FROM incidents WHERE status='open' ORDER BY last_seen DESC LIMIT 20")
    rows = [dict(r) for r in cur.fetchall()]
    cur.close(); conn.close()
    return rows

def get_all_incidents(services: list[str] | None = None) -> list[dict]:
    conn = get_conn(); cur = conn.cursor()
    if services:
        cur.execute("SELECT * FROM incidents WHERE services && %s ORDER BY last_seen DESC", (services,))
    else:
...
def get_incident_by_id(inc_id: str) -> dict | None:
    conn = get_conn(); cur = conn.cursor()
...
def resolve_incident(inc_id: str) -> dict | None:
    conn = get_conn(); cur = conn.cursor()
    cur.execute("UPDATE incidents SET status='resolved' WHERE inc_id=%s RETURNING *", (inc_id,))
    row = cur.fetchone(); conn.commit(); cur.close(); conn.close()
    return _row_to_dict(row) if row else None

def save_incident(result: dict, logs: list[dict]) -> dict:
    ...
    if not dup_id:
        cur.execute("SELECT inc_id FROM incidents WHERE title=%s AND status='open'", (result["title"],))
        row = cur.fetchone()
        if row: dup_id = row["inc_id"]

    if dup_id:
        cur.execute("SELECT timeline FROM incidents WHERE inc_id=%s", (dup_id,))
        tl = cur.fetchone()["timeline"]
        ...
        cur.execute("""UPDATE incidents SET occurrences=occurrences+1, last_seen=NOW(),
                       latest_logs=%s, timeline=%s WHERE inc_id=%s RETURNING *""",
                    (json.dumps(logs), json.dumps(timeline), dup_id))
        row = cur.fetchone(); conn.commit(); cur.close(); conn.close()
    ...
    if cascade_root and isinstance(cascade_entry, dict):
        cur.execute("SELECT cascades, timeline FROM incidents WHERE inc_id=%s", (cascade_root,))
        root = cur.fetchone()
        if root:
            ...
            cur.execute("""UPDATE incidents SET cascades=%s, timeline=%s, last_seen=NOW()
                           WHERE inc_id=%s RETURNING *""",
                        (json.dumps(cascades), json.dumps(timeline), cascade_root))
            row = cur.fetchone(); conn.commit(); cur.close(); conn.close()
    ...
    cur.execute("""INSERT INTO incidents
        (inc_id,title,severity,status,services,team,hypotheses,evidence,timeline,ai_summary,cascades,latest_logs)
        VALUES (%s,%s,%s,'open',%s,%s,%s,%s,%s,%s,'[]',%s) RETURNING *""",
        (inc_id, result["title"], result["severity"],
         ...
         json.dumps(timeline), result.get("ai_summary", ""), json.dumps(logs)))
    row = cur.fetchone(); conn.commit(); cur.close(); conn.close()
